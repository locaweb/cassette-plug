defmodule Cassette.Plug.RequireRolePlugTest do
  ...
  describe "has_role?/3" do
    test "has_role?/3 returns false when session has no user (when role is a string)", %{
      conn: conn
    } do
    ...
    test "has_role?/3 returns false when session has no user (when role is a function)", %{
      conn: conn
    } do
    ...
    @tag with_user: true
    test "has_role?/3 returns false when the user does not have any of the roles listed", %{
      conn: conn
    } do
    ...
    @tag with_user: true
    test "has_role?/3 returns true when the user has the role (when role is a function)", %{
      conn: conn
    } do
    ...
    @tag with_user: true
    test "has_role?/3 returns false when the user does not have the role (when role is a function)",
         %{conn: conn} do
  ...
  describe "has_raw_role?/2" do
    test "has_raw_role?/2 returns false when session has no user (when role is a string)", %{
      conn: conn
    } do
      refute has_raw_role?(conn, "ACME_ADMIN")
    end

    test "has_raw_role?/2 returns false when session has no user (when role is a list)", %{
      conn: conn
    } do
      refute has_raw_role?(conn, ["EXAMPLE_ADMIN", "EXAMPLE_MANAGER"])
    end

    test "has_raw_role?/2 returns false when session has no user (when role is a function)", %{
      conn: conn
    } do
    ...
    @tag with_user: true
    test "has_raw_role?/2 returns false when the user does not have any of the roles listed", %{
      conn: conn
    } do
    ...
    @tag with_user: true
    test "has_raw_role?/2 returns true when the user has the role (when role is a function)", %{
      conn: conn
    } do
    ...
    @tag with_user: true
    test "has_raw_role?/2 returns false when the user does not have the role (when role is a function)",
         %{conn: conn} do
