defmodule Cassette.Plug.DefaultHandlerTest do
  ...
  test "url/2 builds the current request url" do
    conn = %{conn(:get, "/foo?bar=42") | host: "example.org"} |> Conn.fetch_query_params()

    assert DefaultHandler.url(conn, []) == "http://example.org/foo?bar=42"
  end
